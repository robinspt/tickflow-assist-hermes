...
if [ -d "$HERMES_SKILLS_DIR" ]; then
  info "==> 清理旧 TickFlow command skills"
  for legacy_skill_link in "$HERMES_SKILLS_DIR"/ta "$HERMES_SKILLS_DIR"/ta-* "$HERMES_SKILLS_DIR"/ta_*; do
    [ -e "$legacy_skill_link" ] || [ -L "$legacy_skill_link" ] || continue
    if [ -L "$legacy_skill_link" ]; then
      legacy_target="$(readlink "$legacy_skill_link" || true)"
      if [[ "$legacy_target" == "$ROOT_DIR"/skills/ta* ]] || [[ "$legacy_target" == *"tickflow-assist-hermes/skills/ta"* ]]; then
        rm "$legacy_skill_link"
        info "已移除旧 TickFlow skill 链接：$legacy_skill_link"
      fi
    else
      info "检测到 $legacy_skill_link 已存在且不是符号链接；如不再需要旧 TickFlow skill 入口，请手动处理"
    fi
  done
fi
...
info ""
info "安装完成。请重启 Hermes gateway，然后在会话里运行 /plugins 确认 tickflow-assist 已加载。命令入口为 /ta_ 前缀，例如 /ta_watchlist。"
